...
{{< nota de suporte de opções "Por que não oferece anteriores do Hugo?" >>} Como o [Hugodown Render Hooks](https://gohugo.io/getting-started/configuration-markup#markdown-render-hooks) foi criado acima do tema Mark no [Hugo Christmas Edition, suporta apenas versões ](https://gohugo.io/news/0.62.0-relnotes/)**de** versões 02.0. {{< /admonição >}}

{< recursos dicas ser-fw para "A versão do fa:fa-fw Hugo recomendados} Como alguns temas para ser processado:(fa:fa-fw): SCSS processado:(fa:fa-fw): ): CSS versão **do** Hugo para uma melhor experiência. {{< /admonição >>}}
...
Como alternativa, clone este governo para o `themes`macarrão:

```bash
git clone https://github.com/dillonzq/LoveIt.git themes/LoveIt
```

Ou grite um recurso vazio e este recurso de pasta do seu site:
...
Sinta-se à vontade para editar o arquivo de postagem incluindo o conteúdo de amostra e valor do início do arquivo.

{{< aviso >>}} Por padrão as páginas e criadas, como todas as páginas. Você deseja renderizar essas páginas, remover uma propriedade `draft: true`dos dados, definir uma propriedade `draft: false`ou definir / `-D`comando . {{< /admonição >}}`--buildDraftshugo`
...
{{< dica. conteúdo da página >>} Quando você executa os `hugo serve`arquivos é alterado, quando é atualizado automaticamente com as alterações. {{< /admonição >}}

{{<<<}}} Como é usado `.Scratch`no Hugo, é altamente recomendado o tema para que você possa publicar detalhes para a página que você está editando `--disableFastRender`.`hugo server`
...
### 2.6 Construção do Local

Quando seu site estiver pronto para implantação, execute o seguinte comando:

```bash
hugo
```

Uma `public`pasta será produzida, contendo todo o conteúdo estático e ativo do seu site. Agora pode ser instalado em qualquer servidor web.

[{ {](https://www.netlify.com/) < Leia mais dicas de sugestões > publicadas do site [}](https://www.netlify.com/blog/2015/07/30/hosting-hugo-on-netlifyinsanely-fast-deploys/) } Como alternativa, você pode usar o [AWS Amplify](https://gohugo.io/hosting-and-deployment/hosting-on-aws-amplify/) , [Github pages](https://gohugo.io/hosting-and-deployment/hosting-on-github/) , [Render](https://gohugo.io/hosting-and-deployment/hosting-on-render/) e muito mais... {{< /admonition >}}

## 3 Configuração

### 3.1 Configuração do site {#site-configuration}

Além [da configuração Hugo](https://gohugo.io/overview/configuration/) e da configuração do [menu](#basic-configuration) , **LoveIt permite** definir os seguintes parâmetros globais na configuração do seu site ( `config.toml`são considerados do menu).

Por favor, abra o bloco de código abaixo para ver a configuração de amostra completa :(far fa-hand-point-down fa-fw)::
...
{{< aviso >}} Observe que alguns parâmetros são documentação explicada em outras se desta. {{< /admonição >}}

{{< nota} de "ambientes Hugo" > Os padrões são `development`com `hugo serve`e `production`com `hugo`.

Desde que não seja permitido no `development`ambiente local, o **sistema de comentários** , o CD não **ativado** e **a impressão digital** não ativados no `development`ambiente.

Você pode habilitar esses recursos com `hugo serve -e production`. {{< /admonição >}}

{{<} dicaN de configuração2.7 alterações >}} {{< versão 0
...
O arquivo de dados CDN padrão está localizado no `themes/LoveIt/assets/data/cdn/`diretório. Você pode mesmo `assets/data/cdn/`. {{< /admonição >}}

{{< dica de advertência "Dicas sobre configuração social" >}} {{< versão 0.2.0 >}}

Você pode definir diretamente seu ID para obter um link social padrão e seu ícone:
...
O link social gerado é `https://mastodon.technology/@xxxx`.
...
{{< versão 0.2.3 >>}} O conteúdo do post separado por uma tag HTML para melhorar o desempenho da consulta e basicamente implementar a de texto completo   `h2`. é usado para limitar o comprimento máximo do índice da parte que começa com uma tag HTML. {{< /admonição >}}`h3contentLengthh2h3`

{{< dica de algolia"} `index.json`para ativar algo sobre algo}} Você precisa fazer upload de arquivos para pesquisa. Você pode fazer upload dos arquivos `index.json`por navegadores CLI, mas uma ferramenta pode ser melhor. [Algolia Atomic](https://github.com/chrisdmacrae/atomic-algolia) é uma boa escolha. Para ser compatível com o modo multilíngue do Hugo, você precisa fazer upload diferente `index.json`para cada idioma para o índice diferente de algolia, como `zh-cn/index.json`ou `fr/index.json`... {{< /admonition >}}